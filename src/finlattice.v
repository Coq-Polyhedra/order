...
Definition dual_disp (d : disp_t) := Disp (d2 d) (d1 d).
Variable (d : disp_t).
Check (erefl _ : dual_disp (dual_disp d) = d).

Lemma nat_display : unit. Proof. exact: tt. Qed.
Lemma nat_display' : unit. Proof. exact: tt. Qed.

Definition nat_disp := Disp nat_display nat_display'.
Definition bool_disp := Disp BoolOrder.bool_display BoolOrder.bool_display.
Check (erefl _ : dual_disp (dual_disp nat_disp) = nat_disp).*)
...
Definition DualPreLatticeMixin :=
  @PreLattice.Mixin _ (POrder.class [porderType of T^d]) witness
...
Context {disp : unit} {T : tMeetSemilatticeType disp}.

Definition mpremeet & {fset T} := @meet _ T.
...
Definition MeetPreLatticeMixin :=
  @PreLattice.Mixin _ (POrder.class T) top
                    _ _ mpremeet_min mpremeet_inf mpremeet_incr
                    mprejoin_max mprejoin_sumeet mprejoin_decr.

Canonical MeetPreLattice := PreLatticeType T MeetPreLatticeMixin.

End MeetToPreLattice.

Section JoinToPreLattice.

Context {disp : unit} {T : bJoinSemilatticeType disp}.

Definition JoinPreLatticeMixin := @MeetPreLatticeMixin _ [tMeetSemilatticeType of T^d].
Canonical JoinPreLattice := PreLatticeType T^d JoinPreLatticeMixin.

End JoinToPreLattice.

Section FinLattice.

Definition premeet_closed {disp : unit} {T : prelatticeType disp} (S : {fset T}) :=
  [forall x : S, [forall y : S, premeet S (fsval x) (fsval y) \in S]].

Definition prejoin_closed {disp : unit} {T : prelatticeType disp} (S : {fset T}) :=
  [forall x : S, [forall y : S, prejoin S (fsval x) (fsval y) \in S]].

Lemma premeet_closedP {disp : unit} {T : prelatticeType disp} (S : {fset T}) :
...
Lemma prejoin_closedP {disp : unit} {T : prelatticeType disp} (S : {fset T}) :
...
Record finLattice_ (T0 : Type) (b : PreLattice.class_of T0)
                   (T := PreLattice.Pack tt b) := FinLattice {
  elements_ : {fset T};
  _ : [&& premeet_closed elements_,
          prejoin_closed elements_ & elements_ != fset0]
}.

Context {disp : unit} {T : prelatticeType disp}.

Definition finLattice : Type := @finLattice_ T (PreLattice.class T).
Definition elements (S : finLattice) : {fset T} := elements_ S.
Coercion elements : finLattice >-> finset_of.

(* Canonical finLattice_subType := [subType for elements]. *)
Canonical finLattice_subType :=
  @SubType
    {fset T}
    (fun es : {fset T} =>
       [&& premeet_closed es, prejoin_closed es & es != fset0])
    finLattice elements (FinLattice (b:=PreLattice.class T))
    (fun (K : finLattice -> Type) K_S (u : finLattice) =>
       let: FinLattice x Px := u in K_S x Px) vrefl_rect.

Definition finLattice_eqMixin := [eqMixin of finLattice by <:].
Canonical finLattice_eqType := EqType finLattice finLattice_eqMixin.

Definition finLattice_choiceMixin := [choiceMixin of finLattice by <:].
Canonical finLattice_choiceType := ChoiceType finLattice finLattice_choiceMixin.

(*Canonical finLattice_finType (S : finLattice) := [finType of S].*)

(* FIXME: ambiguous path *)
Coercion mem_finLattice (S : finLattice) : {pred T} :=
  fun x : T => x \in elements_ S.
...
Lemma in_finLatticeE (S: {finLattice T}) x : (x \in S) = (x \in elements S).
Proof. by []. Qed.

Definition inE := (@in_finLatticeE, inE).

Definition fmeet (S : {finLattice T}) := premeet S.
Definition fjoin (S : {finLattice T}) := prejoin S.

Lemma finlattice_eqP (S S' : {finLattice T}) : S =i S' <-> S = S'.
...
Context {disp : unit} {T : prelatticeType disp} (S : {finLattice T}).

(* TODO: add (only parsing) notation for [preLattice of (<=:(L^~))] *)
(* TODO: same for semilattices and lattice *)

(* TODO: should we keep the Defined. ? *)
Lemma premeet_closedDual :
  [&& @premeet_closed _ [prelatticeType of T^d] S,
      @prejoin_closed _ [prelatticeType of T^d] S &
      S != fset0 :> {fset _}].
Proof. by case: S => S0 premeet_closedS0; rewrite andbCA. Defined.

Canonical FinLatticeDual : {finLattice [prelatticeType of T^d]} :=
  FinLattice premeet_closedDual.

Lemma dual_fjoinE: \fjoin_FinLatticeDual = \fmeet_S. Proof. by []. Qed.
Lemma dual_fmeetE: \fmeet_FinLatticeDual = \fjoin_S. Proof. by []. Qed.
...
case/and3P: i => _ /prejoin_closedP + _; exact.
Qed.
...
Lemma lefinjoin : forall x y : S, finle y x = (finjoin x y == x).
Proof.
...
Coercion fin_porderType : finLattice >-> POrder.type.
Coercion fin_meetSemilatticeType : finLattice >-> MeetSemilattice.type.
Coercion fin_joinSemilatticeType : finLattice >-> JoinSemilattice.type.
Coercion fin_latticeType : finLattice >-> Lattice.type.
...
Lemma fmeetE (disp : unit) (T : prelatticeType disp) (S : {finLattice T}) :
  \fmeet_S =2 premeet S.
Proof. by []. Qed.

Lemma fjoinE (disp : unit) (T : prelatticeType disp) (S : {finLattice T}) :
  \fjoin_S =2 prejoin S.
Proof. by []. Qed.

Lemma mem_fmeet (disp : unit) (T : prelatticeType disp) (S : {finLattice T}) :
  {in S &, forall x y, \fmeet_S x y \in S}.
Proof.
case: S => S premeet_closeds x y; rewrite !inE fmeetE /= => xS yS.
by case/andP: premeet_closeds => /premeet_closedP + _; exact.
Qed.

Lemma mem_fjoin (disp : unit) (T : prelatticeType disp) (S: {finLattice T}) :
  {in S &, forall x y, \fjoin_S x y \in S}.
Proof. exact: mem_fmeet S^~s. Qed.

Lemma finLattice_prop0 (disp : unit) (T : prelatticeType disp) (S : {finLattice T}): S != fset0 :> {fset _}.
Proof. by case: S => S /= /and3P []. Qed.

Lemma finLattice_leE (disp : unit) (T : prelatticeType disp) (S : {finLattice T}) : forall x y : S,
  x <= y = (fsval x <= fsval y).
Proof. by []. Qed.

Lemma finLattice_meetE (disp : unit) (T : prelatticeType disp) (S : {finLattice T}) : forall x y : S,
  fsval (meet x y) = \fmeet_S (fsval x) (fsval y).
Proof. by move=> x y; rewrite insubdK ?mem_fmeet ?fsvalP. Qed.

Lemma finLattice_joinE (disp : unit) (T : prelatticeType disp) (S : {finLattice T}) : forall x y : S,
  fsval (join x y) = \fjoin_S (fsval x) (fsval y).
...
Lemma leIfl (S : {finLattice T}) : {in S &, forall x y, \fmeet_S x y <= x}.
Proof.
apply: sub_pred2 => x y; move: (@leIl _ S x y).
by rewrite finLattice_leE finLattice_meetE.
Qed.

Lemma leIfr (S : {finLattice T}) : {in S &, forall x y, \fmeet_S y x <= x}.
Proof.
...
Lemma lefI (S : {finLattice T}) :
  {in S & &, forall x y z, (x <= \fmeet_S y z) = (x <= y) && (x <= z)}.
Proof.
apply: sub_pred3 => x y z; move: (@lexI _ S x y z).
by rewrite !finLattice_leE finLattice_meetE.
Qed.

Lemma fmeetC (S : {finLattice T}) : {in S &, commutative (\fmeet_S)}.
Proof.
...
Lemma lefIl (S : {finLattice T}) :
  {in S & &, forall x y z, y <= x -> \fmeet_S y z <= x}.
Proof.
...
Lemma lefIr (S : {finLattice T}) :
  {in S & &, forall x y z, z <= x -> \fmeet_S y z <= x}.
Proof. move=> x y z xS yS zS zlex; rewrite fmeetC //; exact: lefIl. Qed.

Lemma fmeetA (S : {finLattice T}) : {in S & &, associative (\fmeet_S) }.
Proof.
apply: sub_pred3=> x y z; move:(@meetA _ S x y z).
by move/(@congr1 _ _ (@fsval _ S)); rewrite !finLattice_meetE.
Qed.

Lemma fmeetxx (S : {finLattice T}) : {in S, idempotent (\fmeet_S)}.
Proof.
...
Lemma leEfmeet (S : {finLattice T}) :
  {in S &, forall x y, (x <= y) = (\fmeet_S x y == x)}.
Proof.
...
Lemma fjoinC (S : {finLattice T}) : {in S &, commutative (\fjoin_S)}.
Proof. exact: (@fmeetC _ _ S^~s). Qed.

Lemma lefUr (S: {finLattice T}) : {in S &, forall x y, x <= \fjoin_S y x}.
Proof. exact: (@leIfr _ _ S^~s). Qed.

Lemma lefUl (S : {finLattice T}) : {in S &, forall x y, x <= \fjoin_S x y}.
Proof. exact: (@leIfl _ _ S^~s). Qed.

Lemma leUf (S : {finLattice T}) : {in S & &, forall x y z,
  (\fjoin_S y z <= x) = (y <= x) && (z <= x)}.
Proof. exact: (@lefI _ _ S^~s). Qed.

Lemma fjoinA (S : {finLattice T}) : {in S & &, associative (\fjoin_S)}.
Proof. exact: (@fmeetA _ _ S^~s). Qed.

Lemma fjoinxx (S : {finLattice T}) : {in S, idempotent (\fjoin_S)}.
Proof. exact: (@fmeetxx _ _ S^~s). Qed.

Lemma leEfjoin (S : {finLattice T}) :
  {in S &, forall x y, (x <= y) = (\fjoin_S y x == y)}.
Proof.
...
Lemma leUfl (S: {finLattice T}) :
  {in S & &, forall x y z, x <= y -> x <= \fjoin_S y z}.
Proof. exact: (@lefIl _ _ S^~s). Qed.

Lemma leUfr (S : {finLattice T}) :
  {in S & &, forall x y z, x <= z -> x <= \fjoin_S y z}.
...
Definition fbot {disp : unit} {T : prelatticeType disp} (S : {finLattice T}) : T :=
  if fpick S is Some x0 then
    \big[\fmeet_S/x0]_(x <- S) x
  else
    witness.

Definition ftop {disp : unit} {T : prelatticeType disp} (S : {finLattice T}) : T :=
  if fpick S is Some x0 then
    \big[\fjoin_S/x0]_(x <- S) x
  else
    witness.
...
Lemma fjoinf0 {disp} {T : prelatticeType disp} (S : {finLattice T}) : {in S, right_id \fbot_S (\fjoin_S)}.
Proof. by move=> x xS; apply/eqP; rewrite eq_fjoinl ?le0f ?mem_fbot. Qed.

Lemma fjoin0f {disp} {T : prelatticeType disp} (S : {finLattice T}): {in S, left_id \fbot_S (\fjoin_S)}.
...
Lemma fmeetf1 {disp} {T : prelatticeType disp} (S : {finLattice T}) : {in S, right_id \ftop_S (\fmeet_S)}.
Proof. exact: (@fjoinf0 _ _ S^~s). Qed.

Lemma fmeet1f {disp} {T : prelatticeType disp} (S : {finLattice T}) : {in S, left_id \ftop_S (\fmeet_S)}.
...
Lemma fmeet_inf_seq {disp} {T : prelatticeType disp} (S: {finLattice T})
  ...
  {subset r <= S} -> {in S, forall y, F y \in S} -> x \in r -> P x ->
     \big[\fmeet_S / \ftop_S]_(i <- r | P i) F i <= F x.
Proof.
...
Lemma fjoinsP {disp} {T : prelatticeType disp} (S : {finLattice T}) (P : pred T) (F : T -> T) x :
  {in S, forall y, P y -> F y <= x} -> \big[\fjoin_S / \fbot_S]_(y <- S | P y) F y <= x.
Proof. exact: (@fmeetsP _ _ S^~s). Qed.

Lemma fmeet_joins {disp} {T : prelatticeType disp} (S: {finLattice T}) x y :
  x \in S -> y \in S ->
  \fmeet_S x y = \big[\fjoin_S / \fbot_S]_(i <- S | (x >= i) && (y >= i)) i.
...
Context {disp : unit} {T : prelatticeType disp} (S : {finLattice T}) (x0 : S).
...
Coercion tbLatticeType : finLattice >-> TBLattice.type.
...
Lemma prejoin_closed1 {disp} {T : prelatticeType disp} (a : T) :
  prejoin_closed [fset a].
Proof. exact: (@premeet_closed1 _ [prelatticeType of T^d]). Qed.

Context {disp : unit} {T : prelatticeType disp} (a : T).
...
Definition lat1 := FinLattice is_lat1.
...
suff : ((\big[\fmeet_S/ \ftop_S]_(i <- S | a <= i) i) \in S) &&
  (a <= (\big[\fmeet_S/ \ftop_S]_(i <- S | a <= i) i)) by
  case/andP.
...
Definition FinLatInterval {disp} {T : prelatticeType disp} (S: {finLattice T}) a b :
  ...
  FinLattice (@closed_itv _ _ S a b).
...
Lemma dual_itv_r {disp} {T : prelatticeType disp} (S : {finLattice T}) a b :
  ([<a; b>]_S)^~s = [< b ; a>]_S^~s.
Proof.
apply/(@val_inj _ _ finLattice_subType). (* FIXME *)
exact: Interval.dual_itv_fset_eq.
Qed.
...
Lemma dualK (disp : unit) (T : prelatticeType disp) (S : {finLattice T}) :
  (S^~s)^~s = S.
Proof. by exact/val_inj. Qed.

Lemma fbot_dual_r (disp : unit) (T : prelatticeType disp) (S : {finLattice T}) :
  \fbot_(S^~s) = \ftop_S.
Proof. by []. Qed.
